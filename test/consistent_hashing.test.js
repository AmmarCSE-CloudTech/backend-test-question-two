...
describe('ConsistentHashing', function() {
  ...
  it('should get a key for a node', function(done) {
    var cons = new ConsistentHashing(nodes);
    var key = cons.getKey('node2');
    cons.getNode(key).should.equal("node2");
    done();
  });
  it('should add a key for a node', function(done) {
    var cons = new ConsistentHashing(nodes);
    var key = 'some arbitrary key';
    cons.addKey(key, 'node2');
    cons.getNode(key).should.equal("node2");
    done();
  });
});
